...
const motion = function (isStage, targetId) {
    ...
    return `
// category属性：name类的名称 id 类的id (运动 声音 外观) ；color 设置icon的填充色  相当于背景色 ；secondaryColour设置icon边框色相当与border-color
    <category name="%{BKY_CATEGORY_MOTION}" id="motion" colour="#4C97FF" secondaryColour="#3373CC">
        ${isStage ? `
        <label text="${stageSelected}"></label>
        ` : `
        <block type="motion_movesteps">//block 属性:id  block的id;type：block的类型，定义了快的语句形状 参数名
            <value name="STEPS"> //value属性：这个在block定义中设置arg参数类型为“input_value"时才有效;name 参数名，vm的blocks在定义函数的时候，或在生成代码的时候会用到参数。
                <shadow type="math_number">//shadow属性：shadow是输出值的块，它块的定义和moton这些类不一样，里面有个output属性来决定是输出number,还是string,outputShape决定了块的形状。
